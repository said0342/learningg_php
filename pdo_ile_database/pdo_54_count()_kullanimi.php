...
//altta kullanacağımızı rowcount içinde de kullanabilirdik zaten genelde rowcount kullanacağız
$Sorgu = $VeritabaniBaglantisi->query("SELECT COUNT(id) AS toplam FROM uyeler")->fetch(); // AS komutu takma isim takıyor mesela COUNT(id) AS satirsayisi dedik COUNT(id)'deki değeri satirsayisi adlı takma ada atıyor
//count da fetch() kullanıyoruz...

if($Sorgu){

  
  echo "Kayit Sayisi : ".$Sorgu["toplam"];
