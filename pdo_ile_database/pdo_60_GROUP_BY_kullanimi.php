...
$Sorgu = $VeritabaniBaglantisi->query("SELECT * FROM uyeler GROUP BY il ORDER BY il DESC",PDO::FETCH_ASSOC); // burada yaptığım işlem ile tekrar eden verilerin tekrar tekrar yazılmasını önledim isterse 100 tane konya olsun mesela sistem sadece 1 tane konya yazacak
// GROUP BY kullanırken otomatik olarak alfabetik sıralar tabi kendimiz istersek ORDER BY il DESC yazarız tam ters sıralar hatta üstteki satırda bu şekilde kullandım.
// GROUP BY kullanırken istediğim sütunu alabilirim
if($Sorgu){
...
foreach($SorguA as $yazdir){
    echo $yazdir["yas"]."|".$yazdir["adisoyadi"]."<br>"; // istediğimiz sütun özelliğini yazdırabiliriz ben adisoyadi ve yas yazdirdim
}
